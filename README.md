...
**Dataset size:** ~21.8 million rows × 20 columns (before cleaning)
...
├── logistic_regression.ipynb    # Model 1: Logistic Regression (baseline)
├── df_clean.csv                 # Cleaned dataset (generated by EDA notebook)
└── README.md
```
...
- **Handled structural nulls:** The first stop of each service has no arrival time, and the last stop has no departure time — these are expected by design and preserved.
...
- **Delay distribution:** Heavily right-skewed — the vast majority of services run on time or with minimal delays, while a small proportion experience severe delays (> 15 min).
...
- **Class imbalance:** Approximately **9% of services** experience a significant delay (> 5 min) vs. **91% on time** — a major challenge for modeling.
...
1. `StandardScaler` — normalizes numerical features to zero mean and unit variance.
2. `LogisticRegression` with:
   ...
   - `solver='lbfgs'` — efficient for this scale
...
- **AUC-ROC** — overall class separability
- **PR-AUC** (Average Precision) — more informative than ROC for imbalanced data
- **Confusion Matrix**
...
- **Positive coefficients** → increase probability of significant delay
- **Negative coefficients** → decrease probability of significant delay
...
The dataset is heavily imbalanced (≈ 9% delayed vs. 91% on-time). Strategies applied and considered:
